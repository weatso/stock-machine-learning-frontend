    ...
    <>
      ...
      <aside 
        className={cn(
          ...
          // HAPUS 'overflow-hidden' DI SINI AGAR HANDLE BISA KELUAR
          
          isMobile ? "fixed left-0 top-0 transition-transform duration-300" : "relative",
          isMobile && !isOpenMobile ? "-translate-x-full" : "translate-x-0"
        )}
        style={{ width: isMobile ? 280 : width }} 
      >
        
        {/* WRAPPER KONTEN (Beri overflow-hidden di sini supaya teks tidak bocor) */}
        ...
        {!isMobile && (
          <div
            ...
            // Trik: Lebar 16px (w-4) tapi invisible. Posisi -right-2 agar centered di border.
            // Z-Index 50 agar selalu di atas konten apapun.
            className="absolute -right-2 top-0 w-4 h-full cursor-col-resize z-50 flex justify-center group/resizer"
          >
            {/* Indikator Visual (Garis Biru Tipis saat Hover) */}
            <div className="w-0.5 h-full bg-blue-500/0 group-hover/resizer:bg-blue-500 transition-colors delay-75" />
          </div>
        )}
